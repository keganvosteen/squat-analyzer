...
// Container that holds the live video/canvas and can go fullscreen
const VideoContainer = styled.div`
  ...
  /* For portrait orientation, use taller ratio */
  @media (orientation: portrait) {
    aspect-ratio: 3 / 4; /* 3:4 for upright phones */
    max-height: 75vh;
  }
`;
...
// Extra canvas used for drawing pose landmark debug visuals
const PoseDetectionCanvas = styled.canvas`
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
...
const VideoCapture = ({ onFrameCapture, onRecordingComplete }) => {
  // State for camera devices and selection
  const [cameraDevices, setCameraDevices] = useState([]);
  const [selectedCameraId, setSelectedCameraId] = useState('');
  const [showCameraSelector, setShowCameraSelector] = useState(false); // Controls visibility of camera device dropdown
  ...
  const toggleFullscreen = () => {
    if (!isFullscreen) {
      if (containerRef.current && containerRef.current.requestFullscreen) {
        ...
        // fallback – just use internal state styling
  ...
  async function initializeCamera(frontFacing = true, specificDeviceId = null) {
    setError(null);
    setIsLoading(true);
    ...
    try {
      ...
      // Try to find external webcam or use preferred camera
      try {
        ...
        // Use the specific device ID if provided
        if (specificDeviceId) {
          constraints.video.deviceId = { exact: specificDeviceId };
          console.log('Using specific camera device');
        }
        // Otherwise check for a saved preference
        else if (savedCameraId) {
          const savedCamera = cameras.find(c => c.deviceId === savedCameraId);
          if (savedCamera) {
            constraints.video.deviceId = { exact: savedCameraId };
            console.log(`Using saved camera: ${savedCamera.label}`);
          }
        }
        // If no saved preference, look for an external webcam
        else {
          // Try to find external webcam by looking at the label
          const externalCamera = cameras.find(c => 
            c.label && c.label.toLowerCase().includes('external'));
          
          if (externalCamera) {
            constraints.video.deviceId = { exact: externalCamera.deviceId };
            localStorage.setItem('preferredCameraId', externalCamera.deviceId);
            console.log(`Selected external webcam: ${externalCamera.label}`);
          }
        }
      } catch (err) {
        console.warn('Failed to enumerate cameras:', err);
        // If enumeration fails, fall back to simpler device selection
        if (specificDeviceId) {
          constraints.video.deviceId = { exact: specificDeviceId };
          console.log(`Using specific camera device: ${specificDeviceId}`);
        } else {
          // Fallback to facing mode if no device ID is specified
  ...
  // Enumerate available camera devices and select preferred/external one
  const getCameraDevices = useCallback(async () => {
    try {
      // Request permission to access cameras first for labels to be populated
      ...
      // Prefer saved camera or external webcam
      const savedId = localStorage.getItem('preferredCameraId');
      let chosenId = null;
      if (savedId && videoDevices.some(d => d.deviceId === savedId)) {
        chosenId = savedId;
      } else {
        const external = videoDevices.find(d => d.label && d.label.toLowerCase().includes('external'));
        if (external) {
          chosenId = external.deviceId;
          localStorage.setItem('preferredCameraId', external.deviceId);
        } else if (videoDevices.length) {
          chosenId = videoDevices[0].deviceId;
          localStorage.setItem('preferredCameraId', videoDevices[0].deviceId);
        }
      }
      if (chosenId) {
        setSelectedCameraId(chosenId);
        // Auto (re)start stream with chosen camera
        restartCameraWithNewDevice(chosenId);
      }
    } catch (err) {
  ...
  // Run once on mount to populate camera list
  useEffect(() => {
    getCameraDevices();
  }, [getCameraDevices]);

  // Restart stream when user selects a different camera in UI
  const restartCameraWithNewDevice = useCallback(async (deviceId) => {
    try {
  ...
  const handleCameraSelect = useCallback((deviceId) => {
    ...
    restartCameraWithNewDevice(deviceId);
    setShowCameraSelector(false);
  }, [restartCameraWithNewDevice]);
...
if (typeof onRecordingComplete === 'function') {
  ...
  const toggleCameraFacing = () => {
    ...
    initializeCamera(!isFrontFacing);
  };
  ...
  // If selectedCameraId changes and camera not ready, attempt restart
  useEffect(() => {
    if (selectedCameraId && !isCameraReady) {
      restartCameraWithNewDevice(selectedCameraId);
    }
  }, [selectedCameraId, isCameraReady, restartCameraWithNewDevice]);
    ...
    <div className={`video-container ${darkMode ? 'dark-mode' : ''}`} ref={containerRef}>
      ...
      <VideoContainer ref={containerRef} $isFullscreen={isFullscreen}>
        ...
        <CameraSelector 
          ...
          showSelector={showCameraSelector}
          toggleSelector={toggleCameraSelector}
        />
